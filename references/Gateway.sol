    ...
    INotSupportedMethods
{
    ...
    function executeWithERC20(
        ...
        if (!IERC20(token).approve(to, amount))
            revert ApprovalFailed(token, to);
        // Execute the call on the target contract
        // if sender is provided in messageContext call is authenticated and target is Callable.onCall
        // otherwise, call is arbitrary
        if (messageContext.sender == address(0)) {
    ...
    function call(
        ...
        if (payloadSize > MAX_PAYLOAD_SIZE)
            revert PayloadSizeExceeded(payloadSize, MAX_PAYLOAD_SIZE);
    ...
    function _transferFromToAssetHandler(
        ...
        if (token == zetaToken) {
            ...
            if (!IERC20Custody(custody).whitelisted(token))
                revert NotWhitelistedInCustody(token);
    ...
    function _transferToAssetHandler(address token, uint256 amount) private {
        if (token == zetaToken) {
            ...
            if (!IERC20(token).approve(zetaConnector, amount))
                revert ApprovalFailed(token, zetaConnector);
            // send tokens to connector
            ZetaConnectorBase(zetaConnector).receiveTokens(amount);
        } else {
            // transfer to custody
